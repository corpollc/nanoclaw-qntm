...
function parseIdentityJsonText(raw: string): Identity {
  ...
  return {
    privateKey: decodeIdentityBytes(
      value.private_key ?? value.privateKey,
      'private_key',
    ),
    publicKey: decodeIdentityBytes(
      value.public_key ?? value.publicKey,
      'public_key',
    ),
    keyID: decodeIdentityBytes(
      value.key_id ?? value.keyID ?? value.kid,
      'key_id',
    ),
  };
}
...
function loadStoredConversationRecords(
  identityDir: string,
): StoredConversationRecord[] {
  const recordsPath = join(identityDir, 'conversations.json');
  const raw = JSON.parse(readFileSync(recordsPath, 'utf-8')) as unknown;
  if (!Array.isArray(raw)) {
    throw new Error(`invalid qntm conversations file: ${recordsPath}`);
  }
  return raw as StoredConversationRecord[];
}

function parseStoredConversationRecord(
  ...
  const participants = Array.isArray(record.participants)
    ? record.participants.map((entry, index) =>
        decodeIdentityBytes(entry, `participant ${index}`),
      )
    : [];
  ...
  return {
    ...
    currentEpoch: parseStoredConversationEpoch(
      record.currentEpoch ?? record.current_epoch,
    ),
  };
}

export function loadQntmConversationFromDir(
  identityDir: string,
  convId: string,
): Conversation {
...
export function decodeQntmBody(
  bodyType: string,
  body: Uint8Array,
): {
  rawBody: string;
...
export function decodeInboundEnvelope(params: {
  ...
  return decryptMessage(envelope, params.conversation);
}
