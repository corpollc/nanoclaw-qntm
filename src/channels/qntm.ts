...
function resolveRegisteredQntmGroups(
...
function isSelfAuthoredMessage(
  ...
  return Boolean(
    senderPublicKey && senderPublicKey === toComparableHex(identity.publicKey),
  );
}
...
export class QntmChannel implements Channel {
  ...
  private readonly log: Pick<typeof logger, 'info' | 'warn' | 'error' | 'debug'>;
  ...
  private async handleInbound(params: {
    ...
    const { bodyForAgent } = decodeQntmBody(
      message.inner.body_type,
      message.inner.body,
    );
  ...
  async connect(): Promise<void> {
    ...
    for (const entry of qntmGroups) {
      ...
      const subscription = this.client.subscribeMessages(
        conversation.id,
        initialCursor,
        {
          getCursor: () =>
          ...
          onError: (error) => {
            this.log.warn(
              `qntm: subscription error for ${entry.conversationId}: ${error.message}`,
            );
          },
        },
      );
    ...
    this.connected = true;
    this.log.info({ count: this.subscriptions.size }, 'qntm channel connected');
  }
  ...
  async disconnect(): Promise<void> {
    ...
    this.log.info('qntm channel disconnected');
  }
}
...
export function createQntmChannelFactory(deps?: QntmChannelDeps) {
  return (opts: ChannelOpts): Channel | null => {
    ...
    const envFileValues =
      deps?.envFileValues ??
      readEnvFile(['QNTM_IDENTITY_DIR', 'QNTM_RELAY_URL']);
    const identityDir =
      env.QNTM_IDENTITY_DIR?.trim() || envFileValues.QNTM_IDENTITY_DIR?.trim();
    if (!identityDir) {
      logger.warn('qntm: QNTM_IDENTITY_DIR not set');
      return null;
    }
    const relayUrl =
      env.QNTM_RELAY_URL?.trim() ||
      envFileValues.QNTM_RELAY_URL?.trim() ||
      DEFAULT_RELAY_URL;
    const cursorStore =
      deps?.cursorStore ??
      createFileCursorStore({
